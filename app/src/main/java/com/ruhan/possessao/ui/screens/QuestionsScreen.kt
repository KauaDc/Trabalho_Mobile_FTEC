...
fun QuestionsScreen(
    ...
    val allQuestions = listOf(
        ...
        "temperature_shift" to "A temperatura corporal ou do ambiente varia sem motivo aparente?",
        "object_movement" to "Objetos se movem levemente próximos à pessoa?",
        "memory_gaps" to "Existem lapsos de memória durante momentos de alteração de comportamento?",
        "shadow_presence" to "Alguém relata ver sombras ou vultos próximos à pessoa?",
        "mirror_discomfort" to "Há incômodo ou recusa em olhar o próprio reflexo?",
        "unusual_strength" to "A pessoa demonstra força incomum em certos episódios?",
        "animal_reaction" to "Animais agem de forma inquieta ou agressiva perto da pessoa?",
        "time_distortion" to "A percepção do tempo parece mudar durante os episódios?",
        "persistent_whispers" to "A pessoa relata ouvir sussurros ou vozes sem origem clara?",
        "symbolic_drawings" to "Ela faz desenhos ou símbolos repetitivos sem entender o motivo?",
        "unexplained_fatigue" to "Sente fadiga extrema após eventos de natureza incomum?"
    )

    fun getRandomQuestions(quantity: Int = 7): List<Pair<String, String>> {
        return allQuestions.shuffled().take(quantity)
    }
